...
- **Discovery state** before typing: the input stays primary, the trailing filter button opens the full search filter sheet, Quick starts provides a short row of time-aware common actions and can include recent-search cuisine affinity, Trending now uses compact suggestion chips, Trending dishes shows a horizontal row of the top 10 dishes ranked by 7-day saves and posts, Popular collections and Popular places support decision-making, and creator suggestions sit lower on the page.
- **Results state** after typing or using Nearby: All / Dishes / Collections / Places / Posts / People tabs organize the same ranked result sets. All is a presentation layer that shows the most relevant food entity for the query without changing the underlying ranking math. Results update from the controlled search text as the user types; network/provider calls remain debounced, but the UI should never require pressing return to refresh.
...
Nearby controls stay quiet until the user asks for them. Tapping the Nearby quick start switches search into around-me mode, shows a single area/radius token, and opens a filter sheet for current location, manual suburb/postcode, and 2 km / 5 km / 10 km / 25 km / 50 km radius choices. Search still never requests GPS on mount.

The same sheet owns the v1 filters: Location, Food, Venue, Needs, and Sort. Sort options are Best Match, Nearby, Popular, and Newest. Price, rating, saved-count, and advanced filters are intentionally out of scope. Active filters render as compact tokens under the input only when set.
