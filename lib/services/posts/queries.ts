...
export function extractPostRow(p: RawPost | null | undefined): SavedPostRow | null {
  ...
  return {
    ...
    media: sortedPhotos.map((ph) => ({
      ...
      processed_url: ph.processed_url ?? null,
    ...
    place_name: r?.name ?? null,
    place_address: r?.address ?? null,
    place_lat: r?.latitude ?? null,
    place_lng: r?.longitude ?? null,
    place_google_id: r?.google_place_id ?? null,
