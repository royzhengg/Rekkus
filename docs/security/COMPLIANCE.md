...
| OpenStreetMap | Active — place database seed (AU, ~80k–200k venues) | open_osm / ODbL | "© OpenStreetMap contributors" attribution required on map/address display; ODbL allows commercial use; ADR-021 | Required when displaying OSM-sourced place data | `canonical_source = 'osm'` on imported rows; attribution string in all place detail screens | OSM update rules: never overwrite community_verified / owner_verified; delta refresh respects verification_level |
