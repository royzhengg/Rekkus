...
| `search_result_click`              | `post` / `place` / `collection` / `user` / `dish` | entity id     | `{ query, result_type, result_position, search_session_id }`                                         | User taps a ranked search result |
...
| `search_filter_sheet_opened`       | null                           | null          | `{ filter_count }`                                                                                   | User opens the Search filter sheet                       |
| `search_suggestion_selected`       | null                           | null          | `{ suggestion_type, suggestion_id, suggestion_slug, position }`                                      | User chooses a typed Search suggestion                   |
| `search_no_results`                | null                           | null          | `{ query, result_type?, ranking_version? }`                                                          | Domain search returns no results                         |
...
| `search_saved_search_used`         | null                           | null          | `{ query, search_session_id? }`                                                                      | User runs a saved search                                 |
...
| `follow_request_state_changed`     | null                           | null          | `{ action }` — action values: `sent`, `approved`, `declined`, `approved_immediate`, `approved_bulk`, `declined_bulk`, `approved_auto_public` | Private-account follow request workflow changes          |
| `privacy_setting_changed`          | null                           | null          | `{ setting, enabled }` — setting values: `private_account`, `show_activity_status`                   | User changes private account or activity visibility      |
| `blocked_accounts_screen_viewed`   | null                           | null          | null                                                                                                 | User opens Blocked accounts settings                     |
| `blocked_accounts_search_used`     | null                           | null          | null                                                                                                 | User enters a non-empty Blocked accounts search query once per screen session |
| `profile_follow_list_opened`       | `user`                         | user.id       | `{ list_type }`                                                                                      | User opens a profile's Followers or Following list       |
| `profile_interaction`              | `user`                         | user.id       | `{ action, profile_tab?, position?, source?, visible_count? }` — action values: `top_restaurant_tapped`, `manage_top_spots_tapped`, `top_spot_added`, `top_spot_removed`, `top_spot_reordered`, `top_spots_saved`, `tab_selected`, `share_profile_tapped`, `empty_review_cta_tapped` | User uses profile tabs, Top Spots, manage-top-spots screen, profile share, or empty-profile CTAs |
...
| `login_oauth_started`              | null                           | null          | `{ provider }` — provider values: `google`, `apple`                                                  | OAuth/provider sign-in begins                            |
| `login_oauth_success`              | null                           | null          | `{ provider }` — provider values: `google`, `apple`                                                  | OAuth/provider sign-in succeeds                          |
| `login_oauth_failed`               | null                           | null          | `{ provider, reason }` — reason values: `cancelled`, `missing_token`, `network`, `provider_error`, `session_error`, `unknown` | OAuth/provider sign-in fails or is cancelled             |
| `account_linked`                   | null                           | null          | `{ provider }` — provider values: `email`, `google`, `apple`                                        | User links a sign-in provider                            |
| `account_unlinked`                 | null                           | null          | `{ provider }` — provider values: `email`, `google`, `apple`                                        | User unlinks a sign-in provider                          |
...
| File                                            | Event                                                                                     | When                                                            |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------- | --------------------------------------------------------------- |
| `features/search/SearchScreen.tsx`              | `search_query`, `search_result_click`, `place_click`, `collection_interaction`, `no_results_shown`, `no_results_suggestion_click`, `search_filter_sheet_opened`, `search_suggestion_selected`, `search_no_results`, `search_saved`, `search_unsaved`, `saved_search_selected`, `search_saved_search_used` | Query debounces, user taps search results/staff picks, opens filters, chooses suggestions, saves/reruns saved searches, or uses no-results recovery |
...
| `features/settings/BlockedAccountsScreen.tsx`   | `blocked_accounts_screen_viewed`, `blocked_accounts_search_used`                         | User reviews or searches their blocked account list             |
...
| 2026-06-01 | Trending dishes row added to DiscoveryPage (B-549): `fetch_trending_dishes` RPC ranks dishes by 7-day `saved_dishes` + `posts.dish_id` activity; taps fire `dish_view`. | First dish signal on every pre-search surface. |
...
| 2026-06-25 | Search Domain analytics added: `search_filter_sheet_opened`, `search_suggestion_selected`, `search_no_results`, and `search_saved_search_used` (B-637). | Measures filters, typeahead, no-result rate, and saved-search reuse without raw result payloads, provider payloads, or precise coordinates. |
| 2026-06-23 | Apple Sign-In and provider-agnostic connected-account analytics added: `login_oauth_started`, `login_oauth_success`, `login_oauth_failed`, `account_linked`, `account_unlinked` (B-619). | Measures auth/provider funnel health without storing emails, tokens, or raw provider payloads. |
