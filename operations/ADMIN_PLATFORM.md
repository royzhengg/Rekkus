...
| Merge duplicate restaurants | Data owner | `restaurant_merge_events`, `restaurant_aliases`, `restaurant_audit_events` | Record canonical/merged IDs, confidence, before/after summaries, and rollback reference | Use alias/merge history to restore references or supersede merge | Manual review before bulk merge tooling |
| Edit restaurant metadata | Data owner | `restaurants`, `restaurant_observations`, `restaurant_audit_events`, `data_repair_events` | Promote bounded corrections with provenance and audit evidence | Reapply prior value from audit/repair summary | Local DB first, provider refresh second |
...
| Restaurant verification tooling | Restaurant/data owner | `restaurant_ownership_events`, `restaurant_sources`, `restaurant_audit_events` | Review claim evidence, approve/reject/transfer ownership | Supersede ownership event or remove owner | Manual workflow until restaurant portal exists |
...
| Restaurants | Canonical restaurant ID, pending ownership event, merge event, repair status | `restaurant_ownership_events`, `restaurant_merge_events`, `data_repair_events` |
