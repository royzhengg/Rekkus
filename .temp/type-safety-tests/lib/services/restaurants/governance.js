...
exports.recordRestaurantObservation = recordRestaurantObservation;
exports.submitRestaurantEditSuggestion = submitRestaurantEditSuggestion;
exports.submitDuplicateRestaurantSuggestion = submitDuplicateRestaurantSuggestion;
exports.submitCommunityVerification = submitCommunityVerification;
exports.recordRestaurantAuditEvent = recordRestaurantAuditEvent;
exports.submitRestaurantClaim = submitRestaurantClaim;
exports.recordRestaurantAlias = recordRestaurantAlias;
exports.recordRestaurantMergeEvidence = recordRestaurantMergeEvidence;
exports.reportDataRepair = reportDataRepair;
const supabase_1 = require("@/lib/supabase");
async function recordRestaurantObservation(input) {
    const { data: userData } = await supabase_1.supabase.auth.getUser();
    const userId = userData.user?.id;
    if (!userId)
        return;
    await supabase_1.supabase.from('restaurant_observations').insert({
...
async function submitRestaurantEditSuggestion(input) {
...
async function submitDuplicateRestaurantSuggestion(input) {
...
async function submitCommunityVerification(input) {
    await recordRestaurantObservation({
        restaurantId: input.restaurantId,
...
async function recordRestaurantAuditEvent(input) {
    await supabase_1.supabase.from('restaurant_audit_events').insert({
        actor_type: 'client',
        action: input.action,
...
async function submitRestaurantClaim(input) {
    const { data: userData } = await supabase_1.supabase.auth.getUser();
    const userId = userData.user?.id;
    if (!userId)
        return;
    await supabase_1.supabase.from('restaurant_ownership_events').insert({
        restaurant_id: input.restaurantId,
...
async function recordRestaurantAlias(input) {
    const { data: userData } = await supabase_1.supabase.auth.getUser();
    const userId = userData.user?.id;
    if (!userId)
        return;
    await supabase_1.supabase.from('restaurant_aliases').insert({
        restaurant_id: input.restaurantId,
...
async function recordRestaurantMergeEvidence(input) {
    const { data: userData } = await supabase_1.supabase.auth.getUser();
    await supabase_1.supabase.from('restaurant_merge_events').insert({
...
async function reportDataRepair(input) {
    const { data: userData } = await supabase_1.supabase.auth.getUser();
    const userId = userData.user?.id;
    if (!userId)
        return;
    await supabase_1.supabase.from('data_repair_events').insert({
        entity_type: input.entityType,
        entity_id: input.entityId ?? null,
        restaurant_id: input.restaurantId ?? null,
        actor_id: userId,
